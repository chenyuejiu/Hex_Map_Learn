...
public class HexGridChunk : MonoBehaviour {
    ...
    private void TriangulateOpenWater(HexDirection direction, HexCell cell, HexCell neighbor, Vector3 center) {
        ...
        if (direction <= HexDirection.SE && neighbor != null) {
            ...
            if (direction <= HexDirection.E) {
                HexCell nexNeighbor = cell.GetNeighbor(direction.Next());
                if (nexNeighbor == null || !nexNeighbor.IsUnderwater) {
                    return;
                }
                water.AddTriangle(c2, e2, c2 + HexMetrics.GetWaterBridge(direction.Next()));

                indices.z = nexNeighbor.Index;
	...
	void TriangulateCornerTerraces (
		...
		//terrain.AddTriangleColor(weights1, c3, c4);
  //      terrain.AddTriangleTerrainTypes(types);
   ...
   //         terrain.AddQuadTerrainTypes(types);
        ...
        //terrain.AddQuadColor(c3, c4, weights2, weights3);
        //      terrain.AddQuadTerrainTypes(types);
    }
	...
	void TriangulateCornerTerracesCliff (
		...
		if (leftCell.GetEdgeType(rightCell) == HexEdgeType.Slope) {
			...
			//terrain.AddTriangleColor(
   //             weights2, weights3, boundaryColor
   //         );
   ...
   //         terrain.AddTriangleTerrainTypes(types);
	...
	void TriangulateBoundaryTriangle (
        ...
        //terrain.AddTriangleColor( beginWeights, c2, boundaryWeights);
        //      terrain.AddTriangleTerrainTypes(types);
        ...
        for (int i = 2; i < HexMetrics.terraceSteps; i++) {
            ...
            //terrain.AddTriangleColor(c1, c2, boundaryWeights);
            //         terrain.AddTriangleTerrainTypes(types);
        }

		terrain.AddTriangleUnperturbed(v2, HexMetrics.Perturb(left), boundary);
        terrain.AddTriangleCellData(indices, w2, leftWeights, boundaryWeights);
        //terrain.AddTriangleColor(c2, leftWeights, boundaryWeights);
        //      terrain.AddTriangleTerrainTypes(types);
    }
